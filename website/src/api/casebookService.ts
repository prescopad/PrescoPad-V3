 ...
 */
export async function getCasebookPdfUrl(patientId: string): Promise<string> {
  const { data, error } = await supabase.functions.invoke('generate-casebook-pdf', {
    body: { patientId },
  });
  if (error || !data?.url) throw new Error(error?.message || 'Failed to generate casebook PDF.');
  return data.url as string;
}

/** Fetches the generated casebook PDF as a Blob, ready for a download link. */
export async function downloadCasebookPdf(patientId: string): Promise<Blob> {
  const url = await getCasebookPdfUrl(patientId);
  const response = await fetch(url);
  if (!response.ok) throw new Error('Failed to download casebook PDF.');
