...
function mapPrescription(row: Record<string, unknown>): Prescription {
  ...
  return {
    ...
    isMlc: (row.is_mlc as boolean) ?? (row.isMlc as boolean) ?? false,
    attachCertificate: (row.attach_certificate as any) ?? (row.attachCertificate as any) ?? undefined,
    attachReceipt: (row.attach_receipt as any) ?? (row.attachReceipt as any) ?? undefined,
    vitals: (row.vitals as any) ?? null,
    medicines,
    labTests,
...
export async function createPrescription(draft: PrescriptionDraft, doctorId: string): Promise<Prescription> {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      attach_certificate: draft.attachCertificate || null,
      attach_receipt: draft.attachReceipt || null,
