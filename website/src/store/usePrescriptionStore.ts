...
export const usePrescriptionStore = create<PrescriptionStore>((set, get) => ({
  ...
  updateDraft: (partial) => {
    set((state) => ({ currentDraft: { ...state.currentDraft, ...partial } }));
  },

  addMedicine: (med) => {
    set((state) => ({
      currentDraft: { ...state.currentDraft, medicines: [...state.currentDraft.medicines, med] },
    }));
  },
  ...
  addLabTest: (test) => {
    set((state) => ({
      currentDraft: { ...state.currentDraft, labTests: [...state.currentDraft.labTests, test] },
    }));
  },
