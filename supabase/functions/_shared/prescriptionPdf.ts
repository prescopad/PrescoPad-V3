...
export async function renderPrescriptionPdf(input: PrescriptionPdfInput): Promise<Uint8Array> {
  ...
  if (input.medicines?.length) {
    ...
    input.medicines.forEach((m, idx) => {
      ...
      for (let c = 0; c < cells.length; c++) {
        const truncated = fontRegular.widthOfTextAtSize(cells[c], 9) > colWidths[c] - 10
          ? cells[c].slice(0, Math.max(3, Math.floor((colWidths[c] - 10) / 5))) + "…"
          : cells[c];
  ...
  if (input.labTests?.length) {
    ...
    for (const t of input.labTests) {
      newPageIfNeeded(16);
      const testName = t.testName || t.test_name || "";
      const line = t.notes ? `•  ${testName} — ${t.notes}` : `•  ${testName}`;
